 ...
 * 其中服务命名空间+服务名作为该服务下线程池下的所有线程的名字，解决：传入自定义ThreadFactory：ServiceThreadFactory
 * threadPool为：该服务对应的线程池的描述信息
 *（其中threadPool还可以为：scheduledThreadPool——定时？暂无该需求：singleThreadExecutor单线程执行？或可抽象为：ExecutorIdentity）
 * tasks为：线程池中运行的任务列表
